...
//    if (open) {
//        emit sendmessage("Please close the currently open database first");
//        return;
//    }
    QString pathName = "DBMS\\" + dataBaseName;
...
void myDBMS::closeDataBase(QString dataBaseName) {//和析构函数相同
    ...
    QString pathName = "D:\\dbms-master\\build-DBMS-Desktop_Qt_5_9_9_MSVC2015_32bit-Debug\\DBMS\\database" + dataBaseName;
    if (QDir(pathName).exists() == false)printf("The database does not exist");
    else {
...
void myDBMS::myCreateDataBase(QString dataBaseName) {//建立数据库
    ...
    QFile file("D:\\dbms-master\\build-DBMS-Desktop_Qt_5_9_9_MSVC2015_32bit-Debug\\DBMS\\database\\"+dataBaseName+".txt");//同步创建数据库预留表
...
bool myDBMS::DelDir(const QString &path,QString dataBaseName)
{
    ...
    QFile filedatabase;
    filedatabase.setFileName("D:\\dbms-master\\build-DBMS-Desktop_Qt_5_9_9_MSVC2015_32bit-Debug\\DBMS\\database\\" + dataBaseName+".txt");

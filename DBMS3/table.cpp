...
bool Table::constraint()
{
    ...
    if(target)//是否结束操作
    {
        QFile fcache("D:\\dbms-master\\FILE\\database\\"+databasename+"\\data\\"+tableName+".txt");//清空
        fcache.open(QFile::WriteOnly|QFile::Truncate);
        fcache.close();
        QFile ffcache("D:\\dbms-master\\FILE\\database\\"+databasename+"\\data\\"+tableName+".txt");//还原
...
void Table::backups()
{
    QFile cache("D:\\dbms-master\\FILE\\database\\"+databasename+"\\data\\"+tableName+".txt");//从缓存文件中获取之前的语句内容并还原

...
{
     QString fileName = QFileDialog::getOpenFileName(this, tr("Open File"),"D:\\dbms-master\\FILE\\database\\"+databasename+"\\table\\",tr("txt (*.txt)"));//打开文件
}
...
void MainWindow::on_readdb_triggered()
{
    ...
    {;
        ...
        for(int j=0;j<files.size();j++)
        {
            ...
            seclevel.clear();//清空队列
            QFile file("D:\\dbms-master\\FILE\\database\\table\\"+cache+".txt");
            bool ok = file.open(QIODevice::ReadWrite);//
            file.close();
        }
    }

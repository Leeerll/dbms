    ...
    ui(new Ui::login)
{
    ...
    ser->listen(QHostAddress::AnyIPv4,3030);
...
void login::mysystemDataBase() {//建立数据库
    QString pathName = "D:\dbms-master\FILE\database" ;
...
bool isFileExist()
{
    QString fullFileName = "D:\dbms-master\FILE\user\data\user.txt";
...
void login::mysystemuser(){
    if(!isFileExist()){
        QFile file("D:\dbms-master\FILE\user\data\user.txt");
...
void login::readtxt()
{
    QFile f("D:\dbms-master\FILE\user\data\user.txt");

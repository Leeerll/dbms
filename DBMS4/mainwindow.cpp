    ...
    try{
           ...
           if(match.hasMatch())
           {
               ...
               if(columnList.length()==paramList.length()){
                   return dao.insertTo(tableName,dataname,columnList,paramList);
           }
           }
           //无表参数
           else{
    ...
    try{
           ...
           if(!sql.contains("where")){
               ...
               if(match.hasMatch())
               {
                   ...
                   return dao.deleteTo(tableName,dataname,judgeList);
               }
               else{
                   throw QString("Does not conform to the grammar standard");
               }
    ...
    try{
          ...
          if(!sql.contains("where")){
              ...
              if(match.hasMatch())
              {
                  ...
                  return dao.updateTo(tableName,dataname,column,value,judgeList);
              }
              else{
                  throw QString("Does not conform to the grammar standard");
              }
            ...
            if(match.hasMatch()){
               ...
               if(sql.contains("order by")){
                ...
                if(match.hasMatch())
                {
                    ...
                    columnList = column.split(",");
                    t = dao.selectTo(dataname,tableList,columnList,judgeList,groupList,orderList,desc);
                     qDebug()<<"asc:";
                    //将表输出
...
{
     QString fileName = QFileDialog::getOpenFileName(this, tr("Open File"),"D:\\dbms-master\\FILE",tr("txt (*.txt)"));//打开文件
}

//创建表
void MainWindow::on_addtable_triggered()
{
    QFile file("D:\\dbms-master\\FILE\\table"+tableNamewyj+".txt");
...
{
    QDir dir("D:\\dbms-master\\FILE\\table");//将目标文件夹下所有文件获取并输出
...
{
    QFile f("D:\\dbms-master\\FILE\\cache\\cache.txt");//从缓存文件中获取之前的语句内容并还原
...
{
    QFile f("D:\\dbms-master\\FILE\\cache\\cache.txt");//将当前输入框内容缓存备份
...
void MainWindow::on_readdb_triggered()
{
    ...
    QDir dirdbs("D:\\dbms-master\\FILE\\database");//获取数据库
...
//    dbsfiles << "*.jpg" << "*.png" <<"*.txt";
    ...
    {;
        ...
        QDir dir("D:\\dbms-master\\FILE\\"+dbsname[0]+"\\table");//获取对应数据库的表
        QStringList nameFilters;
        nameFilters<<"*.txt";//lky << "*.jpg" << "*.png"
        QStringList files = dir.entryList(nameFilters, QDir::Files|QDir::Readable, QDir::Name);
        ...
        for(int j=0;j<files.size();j++)
        {
            ...
            seclevel.clear();//清空队列
            QFile file("D:\\dbms-master\\FILE\\table\\"+cache+".txt");
            bool ok = file.open(QIODevice::ReadWrite);//
            file.close();
        }
    }
...
void MainWindow::on_treeWidget_itemSelectionChanged()
{
    ...
    QDir dir("D://dbms-master//FILE//table");//获取表
    QStringList nameFilters;
    nameFilters <<"*.txt"; //<< "*.jpg" << "*.png"
    QStringList files = dir.entryList(nameFilters, QDir::Files|QDir::Readable, QDir::Name);
    QStringList seclevel;//二级节点 表
...
void MainWindow::read_tableField(QString tableName){
    QString fileName = "D:\\dbms-master\\FILE\\"+dataname+"\\table\\" + tableName;
...
void MainWindow::read_userField(){
    QString fileName = "D:\\dbms-master\\FILE\\user\\data\\user.txt";
    QFile table(fileName);
...
void MainWindow::update_user_file(){
    int i =0;
    QString userName = "D:\\dbms-master\\FILE\\user\\data\\user.txt";
    QFile file(userName);
...
//添加新用户 CREATE USER yby IDENTIFIED BY
...
bool MainWindow::push_user(QString sql){
    ...
    //GRAND CONNECT, RESOURCE, DBA TO yby;授权
        ...
        //具体到表的权限GRAND ALL ON STUDENT TO ;
        else if(match8.hasMatch()){
            QString filename = "D:\\dbms-master\\FILE\\"+dataname+"\\permission\\" +sqlList[3]+".txt";
...
{
    QFile cache("D:\\dbms-master\\FILE\\"+namedatabase+"\\data\\"+namefile+".txt");//从缓存文件中获取之前的语句内容并还原
    cache.open(QIODevice::ReadOnly);
    QTextStream tcache(&cache);
    QString textcache = tcache.readAll();
    cache.close();

    QFile ff("D:\\dbms-master\\FILE\\"+namedatabase+"\\data\\"+namefile+".txt");
    ff.open(QFile::WriteOnly|QFile::Truncate);
    ff.close();

    QString work;
    QFile f("D:\\dbms-master\\FILE\\"+namedatabase+"\\data\\"+namefile+".txt");
    ...
    {
        QFile fcache("D:\\dbms-master\\FILE\\"+namedatabase+"\\data\\"+namefile+".txt");//清空
        fcache.open(QFile::WriteOnly|QFile::Truncate);
        fcache.close();
        QFile ffcache("D:\\dbms-master\\FILE\\"+namedatabase+"\\data\\"+namefile+".txt");//还原
...
//    sql=db->cmd;
        ...
        if (squece[0] == "create") {
            ...
            bool find=false;
            QDir dir("D:\\dbms-master\\FILE\\database");//获取表
            QStringList nameFilters;
            nameFilters<<"*.txt"; //<< "*.jpg" << "*.png"
            QStringList files = dir.entryList(nameFilters, QDir::Files|QDir::Readable, QDir::Name);
...
bool MainWindow::createTable(QString str){
    ...
    // QRegularExpression re("^create\\stable\\s(\\w+)\\s?\\(\\s?((?:[\\w\\s\\w)]+,)*[\\w\\s\\w)]+)\\s?\\)");
   // QRegularExpression re("^create\\stable\\s(\\w+)\\s?\\(\\s?((?:[\\w\\s\\(\\w)]+,)*[\\w\\s\\(\\w)]+)\\s?\\)");
    ...
    if(match.hasMatch()){
        ...
        tableNamewyj=match.captured(1);
        match.captured(2);
        //create table test(no,int PK,sno,number notnull);
...
//    QStringList a=tmp.split(" ");
    ...
    if(str.indexOf(exp)>=0)
    {
        ...
        filenamewyj=tableNamewyj+".txt";
        QDir dir;
        if(!dir.exists("D://dbms-master//FILE//"+dataname+"//table")){
            dir.mkdir("D://dbms-master//FILE//"+dataname+"//table");
        }
        dir="D://dbms-master//FILE//"+dataname+"//table";
        QString path=dir.filePath(filenamewyj);
        QFile file;
        ...
        for (int i=0;i<list.size();i++) {
            if(list.value(i).indexOf(first)>=0){

            out<<first.cap()<<" ";
            qDebug()<<first.cap();
            }
            if(list.value(i).indexOf(second)>=0){
                if(list.value(i).indexOf(PK)>=0){
                    qDebug()<<PK.cap();

                if(PK.cap()=="PK"){
                    out<<second.cap()<<" TRUE FALSE FALSE FALSE FALSE FALSE FALSE RESTRICT\n";
                    filenamewyj=tableNamewyj+"_index.txt";
                    QDir dir3;
                    if(!dir3.exists("D://dbms-master//FILE//"+dataname+"//index")){
                        dir3.mkdir("D://dbms-master//FILE//"+dataname+"//index");
                    }
                    dir3="D://dbms-master//FILE//"+dataname+"//index";
                    QString path3=dir3.filePath(filenamewyj);
                    QFile file3;
                ...
                if(list.value(i).indexOf(notn)>=0){
                    qDebug()<<notn.cap();
                if(notn.cap()=="notnull"){
                ...
                if(list.value(i).indexOf(uniq)>=0){
                    qDebug()<<uniq.cap();
                if(uniq.cap()=="unique"){
    ...
    QDir dir1;
    if(!dir1.exists("D://dbms-master//FILE//"+dataname+"//data")){
        dir1.mkdir("D://dbms-master//FILE//"+dataname+"//data");
    }
    dir1="D://dbms-master//FILE//"+dataname+"//data";
    filenamewyj=tableNamewyj+".txt";
    QString path1=dir1.filePath(filenamewyj);
    QFile file1;
    ...
    file1.close();
    QDir dir4;
    if(!dir4.exists("D://dbms-master//FILE//"+dataname+"//permission")){
        dir4.mkdir("D://dbms-master//FILE//"+dataname+"//permission");
    }
    dir4="D://dbms-master//FILE//"+dataname+"//permission";
    filenamewyj=tableNamewyj+".txt";
    QString path4=dir4.filePath(filenamewyj);
    QFile file4;
...
bool MainWindow::dropTable(QString str){
    ...
    tableNamewyj=list.value(2);
    filenamewyj=tableNamewyj+".txt";
    QDir dir;
    if(!dir.exists("D:\\dbms-master\\FILE\\"+dataname+"\\table")){
        dir.mkdir("D:\\dbms-master\\FILE\\"+dataname+"\\table");
    }
    dir="D:\\dbms-master\\FILE\\"+dataname+"\\table";
    QString path=dir.filePath(filenamewyj);

    qDebug()<<"文件名="<<filenamewyj;
    QFile file;
    file.setFileName(path);
...
bool MainWindow::checkPower(QString tableName){
    QString filename = "D:\\dbms-master\\FILE\\"+dataname+"\\permission\\" +tableName+".txt";
    QFile usertable(filename);
    QStringList strList1;
